...
type HTTP struct {
	svc scholarship.Service
}

// NewHTTP creates new user http service
func NewHTTP(svc scholarship.Service, er *echo.Group) {
	h := HTTP{svc}
	ur := er.Group("/scholarships")
	ur.POST("", h.create)
	ur.GET("", h.list)
	ur.GET("/:id", h.view)
	ur.GET("/:id/export", h.export)
...
func (h *HTTP) export(c echo.Context) error {
	result, err := h.svc.Export(c, c.Param("id"))
	if err = zaplog.ZLog(err); err != nil {
		return err
	}
